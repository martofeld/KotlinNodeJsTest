...
/**
 * A simple task to remove files
 */
open class RemoveFilesTask: DefaultTask() {

    @Input
    lateinit var files: List<String>

    @Input
    var additionalDirectories: List<String> = listOf()

    @TaskAction
    fun run() {
        val filesRemoved = listOfNotNull(
            File(project.buildDir, "externals/${project.rootProject.name}-${project.name}/src").listFiles()?.toList(),
            File(project.buildDir, "externals/${project.name}/src").listFiles()?.toList(),
            additionalDirectories.mapNotNull { File(it).listFiles()?.toList() }.flatten()
        ).flatten().count {
            it.takeIf { it.name in files }?.run { delete() } ?: false
        }

        println("Removed $filesRemoved files for ${project.name}")
    }
...
/**
 * A specific implementation of the Remove task that removes all the files brought by the kotlinx-nodejs lib, and dukat generates anyways
 */
open class DeleteKotlinJsIncludedFilesTask : RemoveFilesTask() {
...
//            "lib.es5.kt",
            ...
            "promises.fs.promises.module_node.kt",
            "promises.node:fs.promises.module_node.kt",
            "fs.fs.rm.module_node.kt",
            // Files shared by multiple modules
